...
Зробив декілька пошукових запитів через FAISS, підготувавши ембеддінги моделлю `sentence-transformers/all-MiniLM-L6-v2`.

**Плюси:**

- Запит `"How to do a graceful shutdown?"` дав хороший результат - топ-1 чанк дійсно відповідає запиту по суті.

**Мінуси:**

- Запит `"How do I skip logging?"` витягнув у топ-1 чанк, що складається з одного короткого речення ("Skip logging") - формально влучив, але як контекст для відповіді майже марний.
- Запит `"How do I define a GET route in Gin?"` промахнувся: правильний чанк з кодом (`r.GET(...)`) не потрапив у топ, натомість запит `"router GET POST example"` (менш природний, ближчий до ключових слів у тексті) дав кращий результат, хоча формально мав більшу відстань. Це свідчить, що проблема не в самому пошуку, а в чанкінгу - варто краще розбивати текст, щоб код і його опис не розʼєднувались і не губились серед сусідніх чанків.